...
{
    EXPECT_THROW(calculate_durations({ "1am", "so_far_so_good", "2:00am", "oh_no_an_extra_label" }),
                 std::invalid_argument);
    EXPECT_THROW(calculate_durations({ "2:30pm", "ok", "3:00pm", "still_good", "5:00pm", "time_to_throw" }),
                 std::invalid_argument);
...
{
    using namespace std::chrono_literals;
    EXPECT_EQ((DurationMap { { "one", 30min }, { "two", 1h } }),
              calculate_durations({ "12:00pm", "one", "12:30pm", "two", "1:30pm" }));
    EXPECT_EQ((DurationMap { { "one", 45min }, { "two", 15min } }),
              calculate_durations({ "4:15pm", "one", "5pm", "two", "5:15pm" }));
    EXPECT_EQ((DurationMap { { "one", 2h }, { "two", 1h } }),
